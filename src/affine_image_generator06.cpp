...
void affine_image_generator06::set_mask(int x_min, int y_min, int x_max, int y_max)
{
  ...
  //cvShowImage("mask",original_image_with_128_as_background);
  //cvWaitKey();
}
...
cv::Mat affine_image_generator06::generate_affine_image(void)
{
  ...
  cvWarpAffine(original_image_with_128_as_background, generated_image, &A,
	       CV_INTER_NN + CV_WARP_FILL_OUTLIERS /* + CV_WARP_INVERSE_MAP*/, cvScalarAll(0));

  if (use_random_background)
    replace_by_noise(generated_image, 0);
